...
const Services = () => {
  ...
  const sertifRef = useFadeUp();
  const ctaRef = useFadeUp();
  const webRef = useFadeUp();
  const mobileRef = useFadeUp();
    ...
    <div className="font-sans text-gray-800 scroll-smooth">
      ...
      <section className="relative text-white bg-cover bg-center"
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-32">
          <span className="inline-block text-xs tracking-widest text-orange-400 bg-orange-400/10 border border-orange-400/30 px-3 py-1 rounded-full mb-5 hero-line-1">
            {t('services_title')}
          </span>
          <h1 className="text-4xl md:text-6xl font-bold leading-tight mb-6 max-w-3xl hero-line-2">
            {t('services_subtitle')}
          </h1>
          <p className="text-gray-300 max-w-xl mb-10 text-lg leading-relaxed hero-line-3">
            {t('services_subtitle_hero')}
          </p>
          <div className="flex flex-wrap gap-4 hero-line-4">
        ...
        <div ref={layananRef} className="anim-fade-up max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {serviceList.map(({ icon: Icon, color, bg, border, titleKey, descKey, features }, idx) => (
              <div key={titleKey}
                className={`group bg-white border ${border} rounded-2xl p-6 card-hover flex flex-col anim-fade-up stagger-${Math.min(idx + 1, 5)}`}>
      ...
      <section className="py-12 bg-white">
        <div className="max-w-7xl mx-auto px-6 space-y-16">
          ...
          <div ref={webRef} className="grid md:grid-cols-2 gap-12 items-center">
            ...
            <div className="anim-fade-right img-zoom relative rounded-2xl overflow-hidden shadow-2xl aspect-video">
          ...
          <div ref={mobileRef} className="grid md:grid-cols-2 gap-12 items-center">
            <div className="order-2 md:order-1 anim-fade-left img-zoom relative rounded-2xl overflow-hidden shadow-2xl aspect-video">
            ...
            <div className="order-1 md:order-2 anim-fade-right">
      ...
      {/* AKADEMI CAROUSEL */}
      <section className="py-20 md:mt-32 bg-gray-900 text-white overflow-hidden">
        <div ref={akademiRef} className="anim-fade-up max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
            ...
            <div className="flex gap-3">
              <button onClick={prev} className="w-10 h-10 rounded-full border border-white/20 hover:bg-white/10 flex items-center justify-center transition-colors">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button onClick={next} className="w-10 h-10 rounded-full border border-white/20 hover:bg-white/10 flex items-center justify-center transition-colors">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[0, 1, 2].map((offset) => {
              ...
              return (
                <div key={offset}
                  className={`relative rounded-2xl overflow-hidden group transition-all duration-500 ${offset === 0 ? "md:col-span-1 row-span-2" : ""}`}>
          ...
          <div className="flex justify-center gap-2 mt-8">
            {akademiCards.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)}
                className={`w-2 h-2 rounded-full transition-all ${i === current ? "bg-orange-500 w-6" : "bg-white/30"}`} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="bg-gradient-to-br from-orange-600 to-orange-700 text-white py-20 text-center overflow-hidden relative ">
        ...
        <div ref={ctaRef} className="anim-fade-up relative z-10 max-w-2xl mx-auto px-6">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link href="https://wa.me/6281122225804"
              className="slide-up-3 inline-block bg-white text-orange-600 font-bold px-8 py-3.5 rounded-lg hover:bg-orange-50 transition-all hover:scale-105 shadow-lg">
              {t('services_cta_button')}
            </Link>
            <Link href="/portfolio"
              className="slide-up-4 btn-outline-hover border-2 border-white/60 px-8 py-3.5 rounded-lg font-semibold">
